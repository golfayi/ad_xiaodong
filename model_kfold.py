...
atest = atest.merge(temp[['account_id', 'account_id_rate', 'account_id_sums', 'account_id_negs']], on='account_id',
                    how='left')
...
for col in ['aid', 'goods_id', 'account_id']:
    ...
    for day in range(10, 25):
        tmp = all_data.loc[(all_data['day'] != day) & (all_data['day'] != 24)].groupby([col], as_index=False)[
            col + '_cnts'].agg({
            col + '_cnts': 'sum'
        })
        tmp1 = all_data.loc[(all_data['day'] != day) & (all_data['day'] != 24)].groupby([col], as_index=False)[
            col + '_sums'].agg({
            col + '_sums': 'sum'
        })
...
for col in ['aid', 'goods_id', 'account_id']:
    ...
    result = pd.DataFrame()
    for day in range(10, 25):
        tmp = all_data.loc[(all_data['day'] < day) & (all_data['day'] >= (day - 5))].groupby([col], as_index=False)[
            col + '_cnts'].agg({
            col + '_cnts': 'sum'
        })
        tmp1 = all_data.loc[(all_data['day'] < day) & (all_data['day'] >= (day - 5))].groupby([col], as_index=False)[
            col + '_sums'].agg({
            col + '_sums': 'sum'
        })
        tmp[colname] = tmp1[col + '_sums'].values / tmp[col + '_cnts'].values
...
for col in ['aid', 'goods_id', 'account_id'] + cross_columns:
    ...
    if col not in cross_columns:
        ...
        if 'goods_id' in col:
            ...
            col1 = 'aid_size'
        for day in range(10, 25):
            tmp = all_data.loc[(all_data['day'] != day) & (all_data['day'] != 24)].groupby([col1], as_index=False)[
                col].agg({
                colname1: 'mean',
                colname2: 'median'
            })
...
for col in ['aid', 'goods_id', 'account_id']:
    ...
    if col not in cross_columns:
        ...
        for day in range(10, 25):
            tmp = \
            all_data.loc[(all_data['day'] < day) & (all_data['day'] >= (day - 5))].groupby([col1], as_index=False)[
                col].agg({
                colname1: 'mean',
                colname2: 'median'
            })
