...
class DeepFM(nn.Module):
    ...
    def __init__(self, feature_sizes, embedding_size=4,
                 hidden_dims=[256, 64, 32, 16], num_classes=10, dropout=[0.5, 0.5, 0.5, 0.5],
        ...
        fm_first_order_Linears = nn.ModuleList(
            [nn.Linear(feature_size, self.embedding_size) for feature_size in self.feature_sizes[:2]])
        fm_first_order_embeddings = nn.ModuleList(
            [nn.Embedding(feature_size, self.embedding_size) for feature_size in self.feature_sizes[2:8]])
